...
class PhocaGalleryCpViewPhocaGalleryM extends JView
{
	...
	public function display($tpl = null) {
		...
		$sU->returnUrl				= 'index.php?option=com_phocagallery&view=phocagallerym&layout=edit&tab='.$this->tmpl['currenttab']['upload'].'&folder='. $this->currentFolder;
		...
		$this->tmpl['su_url']		= JURI::base().'index.php?option=com_phocagallery&task=phocagalleryu.upload&amp;'
								  ...
								  . JUtility::getToken().'=1&amp;viewback=phocagallerym&amp;'
								  .'folder='. $this->currentFolder.'&amp;tab='. $this->tmpl['currenttab']['multipleupload'];
		...
		if((int)$this->tmpl['enablemultiple']  >= 0) {
			...
			$mU->reload				= JURI::base().'index.php?option=com_phocagallery&view=phocagallerym&layout=edit&'
									.$this->session->getName().'='.$this->session->getId().'&'
									. JUtility::getToken().'=1&tab='.$this->tmpl['currenttab']['multipleupload'].'&folder='. $this->currentFolder;
		...
		if((int)$this->tmpl['enablejava']  >= 0) {
			...
			$jU->returnUrl				= JURI::base().'index.php?option=com_phocagallery&view=phocagallerym&layout=edit&tab='
										.$this->tmpl['currenttab']['javaupload'].'&folder='. $this->currentFolder;
			$jU->url					= JURI::base().'index.php?option=com_phocagallery&task=phocagalleryu.javaupload&amp;'
									 .$this->session->getName().'='.$this->session->getId().'&'
									 . JUtility::getToken().'=1&amp;viewback=phocagallerym&amp;tab='.$this->tmpl['currenttab']['javaupload']
									 .'&folder='. $this->currentFolder;
	...
	protected function addToolbar() {
		...
		JToolBarHelper::title( JText::_( 'COM_PHOCAGALLERY_MULTIPLE_ADD' ), 'multiple.png' );
