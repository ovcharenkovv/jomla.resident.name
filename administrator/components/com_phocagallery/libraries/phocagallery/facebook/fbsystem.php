...
{	
	...
	function renderProcessPage($id, $refreshUrl, $countInfo = '') {
		...
		echo '<title>'.JText::_( 'COM_PHOCAGALLERY_FB_UPLOADING_DATA').'</title>'. "\n";
		echo '<link rel="stylesheet" href="'.JURI::base(true).'/components/com_phocagallery/assets/phocagallery.css" type="text/css" />';

		echo '</head>'. "\n";
		echo '<body>'. "\n";
		
		echo '<div style="text-align:right;padding:10px"><a style="font-family: sans-serif, Arial;font-weight:bold;color:#fc0000;font-size:14px;" href="index.php?option=com_phocagallery&task=phocagalleryc.edit&id='.(int)$id.'">' .JText::_( 'COM_PHOCAGALLERY_STOP_UPLOADING_FACEBOOK_IMAGES' ).'</a></div>';
		
		echo '<div id="loading-ext-img-processp" style="font-family: sans-serif, Arial;font-weight:normal;color:#666;font-size:14px;padding:10px"><div class="loading"><div><center>'. JHTML::_('image', 'administrator/components/com_phocagallery/assets/images/icon-loading.gif', JText::_('COM_PHOCAGALLERY_LOADING') ) .'</center></div><div>&nbsp;</div><div><center>'.JText::_('COM_PHOCAGALLERY_FB_UPLOADING_DATA').'</center></div>';
